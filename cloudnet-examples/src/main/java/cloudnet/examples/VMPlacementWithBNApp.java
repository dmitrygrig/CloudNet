 ...
 */
public class VMPlacementWithBNApp {

    private final static Logger LOGGER = LoggerFactory.getLogger(AllocationApp.class);

    private static final int vmNum = 50;
    private static final int pmPerDcNum = 5;
    private static final int iterNum = 1440;
    ...
    public static void main(String[] args) {
        ...
        // Create clock
        SimClock clock = new SimClock(TimeFrame.Minute, calendar.getTimeInMillis());

        // Create cloud
        Cloud cloud = new IaaSCloud(1, clock, new ElasticityManagerFirstFitOptimistic(new AlwaysVmMigrationPolicy()));
//        Cloud cloud = new IaaSCloud(1, clock, new ElasiticityManagerMCDAViolations(clock, new SimplePredictionStrategy()));
        long currentTimestamp = System.currentTimeMillis();
        String outDir = "resources/out/sim/" + currentTimestamp + "_ff/";
        cloud.setMonitor(new PassiveMonitoringSystem(new CsvHistoryWriter(
                outDir + "cloud.csv",
                outDir + "dcs.csv",
                outDir + "pms.csv",
        ...
        Scheduler scheduler = new IaaSScheduler(new VmGeneratorOnce(new VmSpecAzureA1(), vmNum));
        SimEngine engine = new SimEngineSimple(clock, scheduler, cloud, iterNum);
